...
function make_version(size_bytes: number): SharePointFileVersion {
  return {
    version_id: 'v1',
    size_bytes,
    last_modified_at: '2026-01-01T00:00:00Z',
  } as SharePointFileVersion;
}
...
describe('historical version content storage', () => {
  ...
  function make_connector(version: SharePointFileVersion, overrides = {}): SharePointSiteConnector {
    return {
  ...
  it('buffers a version below the streaming threshold', async () => {
    ...
    const outcome = await sync_file_versions(connector, item, 'site-1', 'snap-1', ctx, undefined);
  ...
  it('propagates a storage failure instead of recording it as a Graph failure', async () => {
    ...
    await expect(
      sync_file_versions(connector, item, 'site-1', 'snap-1', ctx, undefined),
