...
function make_ctx_factory(): TenantContextFactory {
  return {
    create: vi.fn().mockResolvedValue({ destroy: vi.fn() } as unknown as TenantContext),
    create_readonly: vi.fn().mockResolvedValue({ destroy: vi.fn() } as unknown as TenantContext),
    create_storage_only: vi
      .fn()
      .mockResolvedValue({ destroy: vi.fn() } as unknown as TenantContext),
  };
}
