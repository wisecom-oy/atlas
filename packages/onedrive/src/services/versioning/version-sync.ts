...
} from '@/services/versioning/version-content-store';
import type { StoredVersionContent } from '@/services/versioning/version-content-store';
import {
  by_version_age,
  is_version_already_captured,
  later_watermark,
} from '@/services/versioning/version-watermark';
...
async function capture_version(
  ...
  try {
    stored = await store_version_content(connector, item, owner_id, ctx, version);
  } catch (err) {
