/**
 * Decorator around RestoreConnector that records every Graph API request
 * to the active GraphRequestCounter (if any).
 *
 * All restore operations target the Outlook service pool (Exchange Online
 * mail, folder, and attachment endpoints). The upload_bytes field is
 * populated for chunk uploads to track progress against the Outlook
 * 150 MB / 5-minute upload window.
 *
...
export class CostTrackingRestoreConnector implements RestoreConnector {
  ...
  async create_mail_folder(
    ...
    return this._inner.create_mail_folder(tenant_id, mailbox_id, display_name, parent_folder_id);
  }
  ...
  async create_message(
    ...
    return this._inner.create_message(tenant_id, mailbox_id, folder_id, message_body);
  }
  ...
  async add_attachment(
    ...
    return this._inner.add_attachment(tenant_id, mailbox_id, message_id, attachment);
  }
  ...
  async create_upload_session(
    ...
    return this._inner.create_upload_session(
      tenant_id,
      mailbox_id,
      message_id,
      file_name,
  ...
  async count_folder_messages(
    ...
    return this._inner.count_folder_messages(tenant_id, mailbox_id, folder_id);
  }
  ...
  async list_folder_messages(
    ...
    return this._inner.list_folder_messages(tenant_id, mailbox_id, folder_id, top);
  }
}
