 ...
 * or Identity pool within the same tenant. Each pool has its own scope,
 * cost model, and enforcement unit.
 *
...
export interface OperationCost {
   ...
   * `sharepoint_onedrive` entries appear only once OneDrive support is added.
   */
  readonly by_service: Partial<Record<GraphServicePool, ServicePoolCost>>;
  /**
   * Request counts broken down by named operation type, for diagnostic use.
